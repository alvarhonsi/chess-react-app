...
import {piece_images, getMoves, getAttacks, pieceIsMovable, isInCheck, isCapturable, getKingPos} from './chessboard/pieces'
import {Effect} from './chessboard/board-tiles'
import {BoardState, CastleAvailability, Piece, ToMove} from './chessboard/board-types'
import {castleAvailability} from './test-positions'
import {boardPositionToIndex, indexToBoardPosition, nextToMove} from './chessboard/utils'
...
const ChessGame = () => {
    ...
    const [movesList, setMovesList] = useState<[string, string, number][]>([['start', startFEN, 0]])
    ...
    return (
        <Flex>
            <ChessBoard boardState={boardState} effectGrid={effectGrid} handleClick={onClick}/>
            <Box
                bg='gray.700'
                w={['md', 'lg', null, 'xl']}
                h='100%'
...
const getMoveNotation = (origin: number, target : number, boardState: BoardState) : string => {
    const piece = boardState.pieces[origin]
